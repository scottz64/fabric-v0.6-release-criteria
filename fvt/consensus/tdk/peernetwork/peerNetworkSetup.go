...
func SetupLocalNetworkWithMoreOptions(
	...
	pwd, _ := os.Getwd()
	if strings.Contains(pwd, "tdk/") {
		// good: it looks like we are probably in a testing subdirectory of tdk; hopefully it is an immediate subdirectory.
		os.Chdir(pwd + "/../automation")
	} else {
		if strings.Contains(pwd, "tdk") {
			// ok, we are IN directory .../tdk, so we can work with that...
			os.Chdir(pwd + "/automation")
		} else {
			fmt.Println("peernetwork/peerNetworkSetup.go SetupLocalNetworkWithMoreOptions(): ERROR: you must be in a subdirectory of tdk/ when running go tests:\ncurrent pwd: ", pwd)
			panic(errors.New("ERROR: first cd to a test directory underneath tdk/ to run go tests"))
		}
	}
