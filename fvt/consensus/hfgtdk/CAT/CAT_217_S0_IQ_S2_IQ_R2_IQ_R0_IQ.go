...
func main() {
	...
	chco2.CurrentTestName = "CAT_217_S0_IQ_S2_IQ_R2_IQ_R0_IQ.go"
...
chco2.Writer = bufio.NewWriter(osFile)
	...
	// CAT_217_S0_IQ_S2_IQ_R2_IQ_R0_IQ.go
	...
	chco2.QueryAllPeers( "STEP 6, after also STOP PEER " + strconv.Itoa(peerNum2) + " and Invokes" )
	chco2.RestartPeers( []int{ peerNum2 } )
	chco2.InvokesUniqueOnEveryPeer()
	chco2.QueryAllPeers( "STEP 9, after restart PEER " + strconv.Itoa(peerNum2) + " and Invokes" )
	chco2.RestartPeers( []int{ peerNum } )
	chco2.InvokesUniqueOnEveryPeer()
	chco2.QueryAllPeers( "STEP 12, after restart PEER " + strconv.Itoa(peerNum) + " and Invokes" )

	//chco2.AllRunningNodesMustMatch = true    	// OPTIONAL. Depends on testcase details and objectives.

	chco2.Invokes( chco2.InvokesRequiredForCatchUp )
	fmt.Println("Sleep extra 30 secs") ; time.Sleep(chco2.SleepTimeSeconds(30))
	chco2.QueryAllPeers( "STEP 9, after Restart PEER " + strconv.Itoa(peerNum) + " and Invokes" )
