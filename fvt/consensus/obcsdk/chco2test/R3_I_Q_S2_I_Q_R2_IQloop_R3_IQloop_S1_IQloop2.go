...
func main() {
	...
	chco2.CurrentTestName = "R3_I_Q_S2_I_Q_R2_IQloop_R3_IQloop_S1_IQloop2.go"
 ...
 */
	...
	// 
	// R3_I_Q_S2_I_Q_R2_IQloop_R3_IQloop_S1_IQloop.go

	chco2.RestartPeers( []int{ 3 } )		// attempt to docker restart a running peer
	chco2.InvokeOnThisPeer( 2, 0 ) 	// 2 invokes, on peer 0
	chco2.QueryAllPeers( "STEP 3" )
	chco2.StopPeers( []int{ 2 } )
	chco2.InvokeOnThisPeer( 1, 0 ) 	// 1 invoke, on peer 0
	chco2.QueryAllPeers( "STEP 6" )
	chco2.RestartPeers( []int{ 2 } )  // now all 4 peers are running again.
	
	i:=0
	numCycles := 12
	for i=1; i <= numCycles; i++ {  // 120 secs, 2 batches every 2 secs
		chco2.InvokeOnThisPeer( 4, 0 )
		chco2.InvokeOnThisPeer( 4, 0 )
		chco2.InvokeOnThisPeer( 4, 0 )
		chco2.InvokeOnThisPeer( 4, 0 )
		chco2.InvokeOnThisPeer( 4, 0 )
		chco2.QueryAllPeers( "STEP 9, query every 10 secs for 2 minutes, cycle " + strconv.Itoa(i) + "/" + strconv.Itoa(numCycles) )
	}

	chco2.RestartPeers( []int{ 3 } )		// attempt to docker restart a running peer

	// With batchsize = 2, Logsize = 40, and each cycle taking 10 secs, we should force peer2 to state change(?)
	// when we reach 20 batches(?) around cycle 2 or 3. Even with batchsize = 500, it should be similar.
	...
	for i=1; i <= numCycles; i++ {
		...
		chco2.InvokeOnThisPeer( 4, 0 )
		chco2.InvokeOnThisPeer( 4, 0 )
		chco2.QueryAllPeers( "STEP 11, query every 10 secs for 5 minutes, cycle " + strconv.Itoa(i) + "/" + strconv.Itoa(numCycles) )
	}

	chco2.StopPeers( []int{ 1 } )		// stop a good peer; confirm if #2 (stopped and restarted) and #3 (restarted twice) sync up
	...
	for i=1; i <= numCycles; i++ {
		...
		chco2.InvokeOnThisPeer( 4, 0 )
		chco2.InvokeOnThisPeer( 4, 0 )
		chco2.QueryAllPeers( "STEP 13, query every 10 secs for 5 minutes, cycle " + strconv.Itoa(i) + "/" + strconv.Itoa(numCycles) )
	}
