...
func main() {
	...
	chco2.CurrentTestName = "countCatchupsOnRestartedPeer.go"
...
chco2.Writer = bufio.NewWriter(osFile)
	...
	chco2.Invokes( 1000 )
	chco2.QueryAllPeers( "STEP 3, after 1000 invokes" )
	...
	chco2.QueryAllPeers( "STEP 5" )


	// Normally: chco2 runs with minimum 2 secs per invoke. At 3600 secs per hour, then max 1800 invokes per hour.
	// For this test, we set it to one sec.
	// Assuming processing rate=40/sec, this test will run with enough delays for about an hour (plus processing time)

	max := 3600    

	i:= 1
	for i = 1; i < max; i++ {
		chco2.InvokeOnThisPeer( 10, peerNum )
		if chco2.TestsCurrentlyPass() {
			fmt.Println("YAY! The restarted peer " + strconv.Itoa(peerNum) + " was down by 1000, and caught up after " + strconv.Itoa(i) + " secs, " + strconv.Itoa(i*10) +" invokes, sent to only that same peer at 10 per second.")
			break
		}
	}
	if (i >= max) {
			fmt.Println("BOO! The restarted peer " + strconv.Itoa(peerNum) + " was down by 1000, and NEVER caught up after " + strconv.Itoa(i) + " secs, " + strconv.Itoa(i*10) +" invokes, sent to only that same peer at 10 per second.")
	}
