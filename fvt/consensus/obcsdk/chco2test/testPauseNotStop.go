...
func main() {
	...
	chco2.CurrentTestName = "testPauseNotStop.go"
...
chco2.Writer = bufio.NewWriter(osFile)
	...
	peerNum := 2
	//peernetwork.PausePeerLocal(chco2.MyNetwork,peer)
	chco2.StopPeers( []int{ peerNum } )
	chco2.Invokes( 10 )
	chco2.QueryAllPeers( "STEP 3, after STOP/PAUSE PEER " + strconv.Itoa(peerNum) )

	if (chco2.Verbose) { fmt.Println("Sleep extra 60 secs for debugging") }
	time.Sleep(chco2.SleepTimeSeconds(60))

	//peernetwork.UnpausePeerLocal(chco2.MyNetwork,peer)
	chco2.RestartPeers( []int{ peerNum } )
	chco2.Invokes( 10 )
	chco2.QueryAllPeers( "STEP 6, after RESTART/UNPAUSE PEER " + strconv.Itoa(peerNum) )



	// chco2.AllRunningNodesMustMatch = true    	// OPTIONAL. Depends on testcase details and objectives.
	chco2.Invokes(500)				// OPTIONAL. Number could vary, based on the testcase.
	chco2.QueryAllPeers( "STEP FINAL, after many invokes")
