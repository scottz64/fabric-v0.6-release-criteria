...
func main() {
	...
	chco2.CurrentTestName = "S2IQ_S1IQR1IQ_xN.go"
...
chco2.Writer = bufio.NewWriter(osFile)
	...
	for i = 1; i < max; i++ {
		...
		chco2.QueryAllPeers( "STEP 6, after " + strconv.Itoa(i) + "/" + strconv.Itoa(max) + " stops and " + strconv.Itoa(numInvokes) + " more invokes" )
		chco2.RestartPeers( []int{ peerNum } )
		chco2.Invokes( numInvokes )
		chco2.QueryAllPeers( "STEP 6, after " + strconv.Itoa(i) + "/" + strconv.Itoa(max) + " restarts and " + strconv.Itoa(numInvokes) + " more invokes" )
	}
	if (i >= max) {
