...
func schedulerTask() {
	...
	go func() {
		...
		defer wg.Done()
	        iAPIArgs_peer0 := []string{"example02", "invoke", "PEER0"}
		k0 := 1
		for k0 <= numReq {
			chaincode.InvokeOnPeer(iAPIArgs_peer0, invArgs0)
	...
	go func() {
		...
		defer wg.Done()
		iAPIArgs_peer1 := []string{"example02", "invoke", "PEER1"}
		k1 := 1
		for k1 <= numReq {
			chaincode.InvokeOnPeer(iAPIArgs_peer1, invArgs0)
	                //time.Sleep(5000 * time.Millisecond)
			k1++
		}
	...
	go func() {
		...
		defer wg.Done()
	        iAPIArgs_peer2 := []string{"example02", "invoke", "PEER2"}
		k2 := 1
		for k2 <= numReq {
			chaincode.InvokeOnPeer(iAPIArgs_peer2, invArgs0)
	                //time.Sleep(5000 * time.Millisecond)
			k2++
		}
	...
	go func() {
		...
		defer wg.Done()
	        iAPIArgs_peer3 := []string{"example02", "invoke", "PEER3"}
		k3 := 1
		for k3 <= numReq {
			chaincode.InvokeOnPeer(iAPIArgs_peer3, invArgs0)
	                //time.Sleep(5000 * time.Millisecond)
			k3++
		}
