...
func main() {
	...
	chco2.CurrentTestName = "CAT_205_S0_IQDQIQ_S1_IQDD.go"
...
chco2.Writer = bufio.NewWriter(osFile)
	...
	// 
	//	These are the OPPOSITE of the default values, as defined in ../chco2/chco2.go.
	...
	// CAT_205_S0_IQDQIQ_S1_IQDD.go

	chco2.StopPeers( []int{ 0 } )
	chco2.Invokes( chco2.InvokesRequiredForCatchUp )
	chco2.QueryAllPeers( "STEP 3, after STOP PEER 0, and Invokes" )
	chco2.DeployNew(2000,2000)
	chco2.QueryAllPeers( "STEP 5, after REDEPLOY new values" )
	chco2.Invokes( chco2.InvokesRequiredForCatchUp )
	chco2.QueryAllPeers( "STEP 7, after more Invokes " )
	chco2.CatchUpAndConfirm()			// OPTIONAL. Depends on testcase details and objectives.

	chco2.StopPeers( []int{ 1 } )
	chco2.Invokes( chco2.InvokesRequiredForCatchUp )
	chco2.QueryAllPeers( "STEP 10, after also STOP PEER 1, and Invokes, but not enough peers for consensus are running." )
	chco2.DeployNew(3000,3000)

	// Query will fail, since there are not enough nodes running for consensus and thus cannot do a full deployment,
	// and our sdk will try to talk to the new network it (but it doesn't exist yet)
	// chco2.QueryAllPeers( "STEP 12, after REDEPLOY new values, but not enough peers for consensus are running." )
	// If we were to deploy with tags, and query the old network, then that could work...

	chco2.DeployNew(4000,4000)

	// chco2.QueryAllPeers( "STEP 14, after REDEPLOY more new values, but not enough peers for consensus are running." )
