...
func main() {
	...
	chco2.CurrentTestName = "CAT_403_S0_IQ_DQIQDQIQ.go"
...
chco2.Writer = bufio.NewWriter(osFile)
	...
	//	chco2.QsMustMatchExpected = false	// Note 1: for many testcases with complicated multiple stops/restarts (eg,
	//						//   when the chain of custody gets broken), due to #2148, we must set this false.
	//						// Note 2: Also, because actual CH increases when we stop VP0 in THIS testcase
	//						//   (but not all testcases), we need to set this false here.
...
chco2.Invokes( chco2.InvokesRequiredForCatchUp )
chco2.QueryAllPeers( "STEP 4, after more Invokes " )

	chco2.DeployNew(1000000, 1000000)  // try the same Init values
	chco2.QueryAllPeers( "STEP 2, after STOP PEER 0, and REDEPLOY with original A & B init values" )
	chco2.Invokes( chco2.InvokesRequiredForCatchUp )
	chco2.QueryAllPeers( "STEP 4, after more Invokes " )

	chco2.DeployNew(5000, 5000)
	chco2.QueryAllPeers( "STEP 6, after REDEPLOY using new values" )
	chco2.Invokes( chco2.InvokesRequiredForCatchUp )
	chco2.QueryAllPeers( "STEP 8, after more Invokes " )
