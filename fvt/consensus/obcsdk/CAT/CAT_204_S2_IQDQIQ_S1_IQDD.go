...
func main() {
	...
	chco2.CurrentTestName = "CAT_204_S2_IQDQIQ_S1_IQDD.go"
...
chco2.Writer = bufio.NewWriter(osFile)
	...
	// CAT_204_S2_IQDQIQ_S1_IQDD.go

	chco2.StopPeers( []int{ 2 } )
	chco2.Invokes( chco2.InvokesRequiredForCatchUp * 2 )
	chco2.QueryAllPeers( "STEP 3, after STOP PEER 2, and Invokes" )
	...
	// Query won't work; our sdk tries to get to the new network, but the deploy didn't go through because not enough nodes are running.
	// chco2.QueryAllPeers( "STEP 12, after REDEPLOY new values, but not enough peers for consensus are running." )

	// If we deploy with tags, and query the old network, then that could work...
