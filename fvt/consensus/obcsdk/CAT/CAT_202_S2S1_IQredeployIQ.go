...
func main() {
	...
	chco2.CurrentTestName = "CAT_202_S2S1_IQredeployIQ.go"
	...
	// CAT_202_S2S1_IQredeployIQ.go

	chco2.StopPeers( []int{ 2, 1 } )
	chco2.InvokeOnEachPeer( chco2.DefaultInvokesPerPeer )
	chco2.QueryAllPeers( "STEP 3, after STOP PEERS 2 and 1, and Invokes" )
	chco2.DeployInit(0)  // on peer 0, deploy using same initial values... so this may be ignored...
	//chco2.InvokeOnEachPeer( chco2.DefaultInvokesPerPeer )
	chco2.InvokeOnThisPeer( chco2.DefaultInvokesPerPeer, 3 )

	// There were not enough nodes for consensus when we deployed, and still not enough now, so
	// the SDK will fail to find the deployment transaction (since it hasn't been executed yet).
	// That is expected, and is ok; this test will pass anyways.
	chco2.QueryAllPeers( "STEP 6, after Deploy and Invoke, without consensus")
