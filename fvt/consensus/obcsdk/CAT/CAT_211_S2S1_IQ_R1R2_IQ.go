...
func main() {
	...
	chco2.CurrentTestName = "CAT_211_S2S1_IQ_R1R2_IQ.go"
...
chco2.Writer = bufio.NewWriter(osFile)
	...
	// CAT_211_S2S1_IQ_R1R2_IQ.go

	chco2.StopPeers( []int{ 2, 1 } )
	chco2.InvokesUniqueOnEveryPeer()
	chco2.QueryAllPeers( "STEP " + strconv.Itoa(3) + ", after STOP PEERS 2, 1, and Invokes" )

	chco2.RestartPeers( []int{ 1, 2 } )

	chco2.Invokes( chco2.InvokesRequiredForCatchUp )
	fmt.Println("Sleep extra 60 secs") ; time.Sleep(chco2.SleepTimeSeconds(60))
	chco2.QueryAllPeers( "STEP 6, after Restarted all the stopped PEERS, and Invokes " )
