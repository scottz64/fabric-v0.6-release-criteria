...
func main() {
	...
	chco2.CurrentTestName = "CAT_114_S1_IQ_R1_IQcatchup.go"
...
chco2.Writer = bufio.NewWriter(osFile)
	...
	// CAT_114_S1_IQ_R1_IQcatchup.go
	// Stop VP1. 1000 Invokes. Query/Verify CH/A/B all exact match in running peers. Restart VP1. Invokes enough to catchup. Query.
	...
	//  We see errors here when querying peer1 for a and b:
	...
	//  Two mins of sleep did not help, although eventually the TEST PASSED whether or not we slept 2 mins.

	chco2.QueryAllPeers( "STEP 5, after RESTART PEER " + strconv.Itoa(peerNum) + ", and 16 more Invokes " )
	chco2.Invokes( chco2.InvokesRequiredForCatchUp )
	chco2.AllRunningNodesMustMatch = true    	// OPTIONAL. Depends on testcase details and objectives.
	chco2.QueryAllPeers( "STEP FINAL, after enough invokes to ensure state transfer and restarted peer should catchup")
