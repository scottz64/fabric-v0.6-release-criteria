...
func main() {
	...
	chco2.CurrentTestName = "CAT_103_S0_IQDQIQ.go"
...
chco2.Writer = bufio.NewWriter(osFile)
	...
	// CAT_103_S0_IQDQIQ.go
	// Stop VP1. Invoke. Query/Verify CH/A/B all exact match in running peers. Deploy new values. Query. Invokes. Query.
	...
	// 								// our expectations; so send more and all are processed and test passes.
	chco2.Invokes( chco2.InvokesRequiredForCatchUp * 2 )
	chco2.QueryAllPeers( "STEP 3, after STOP PEER " + strconv.Itoa(0) + " and Invokes" )
	chco2.DeployNew(500,500)
