...
func CUD_UserRecord(stub shim.ChaincodeStubInterface, tableName string, keys []string, args []byte) error {
	...
	if !ok {
		return errors.New("CUD_UserRecord: InsertRow into " + tableName + " Table failed. Row with given key " + keys[0] + " already exists")
	}
	fmt.Printf("\nCUD_UserRecord: InsertRow into "+tableName+" Table operation Successful")
