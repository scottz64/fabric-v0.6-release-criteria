...
#    @wip
@scat
  Scenario: Peers catch up only when necessary - extensive
    ...
    When requesting "/chain" from "vp0"
    Then I should "store" the "height" from the JSON response
    When requesting "/chain" from "vp1"
    Then I should get a JSON response with "height" = "previous"
    When requesting "/chain" from "vp3"
    Then I should get a JSON response with "height" = "previous"
    ...
    When I query chaincode "example2" function name "query" with value "a" on peers:
            | vp0  | vp1 | vp3 |
    Then I should get a JSON response from peers with "result.message" = "190"
            | vp0  | vp1 | vp3 |
