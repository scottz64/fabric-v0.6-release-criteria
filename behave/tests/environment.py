...
#        for target in ["vp0", "vp1", "vp2", "vp3"]:
#            request_url = "https://{0}/api/com.ibm.zBlockchain/peers/{1}/status".format(context.remote_ip, target)
#            resp = requests.get(request_url, headers=headers, verify=False)
#            print("Status for {0}: {1}".format(target, resp.text.strip()))
#            if resp.text and "STARTED" not in resp.text:
...
#                resp = requests.post(request_url, headers=headers, verify=False)
#                resp.connection.close()
#                time.sleep(120)
#            # Sleep to give the network time to get caught up
#            time.sleep(30)
    else:
